...
仓库不再保留带日期的 YAML 快照文件；需要查看旧版本时使用 GitHub 提交历史。
...
- 更新活动 `.list` 前，先把当前版本另存为对应的 `_YYYYMMDD.list` 快照，再修改无日期活动文件。
- 更新 `Clash.ini` 前，先把当前版本另存为 `Clash_YYYYMMDD.ini`，再修改 `Clash.ini`；快照和活动文件都同步到本仓库。
- `.list` 的日期快照和当前活动文件也要同步到本仓库。
...
仓库不保留带日期的 YAML 快照；`.list` 活动文件的日期快照会继续保留。当前规则以无日期的 `.list` 文件为准。
